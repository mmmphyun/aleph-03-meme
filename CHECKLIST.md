...
- [x] 챌린지 포즈 프리셋 4종 구현 및 전환:
  - `GEOJE_YAHO` (리센느 거제 야호: 양손 입가 확성기 + 젖힌 상체)
  - `CHOI_SAN_BAD` (최산 BAD: 치명적 어깨 꺾기 + 날카로운 턱선)
  - `RONALDO_SIU` (호날두 시우: 공중 착지 후 양팔 뒤로 포효)
  - `CUTE_HEART` (축구장에서 하는 앙증 볼하트)
- [x] 대두(Bobblehead) 슬라이더 (0.8배 ~ 2.5배 실시간 머리 스케일 조절)
- [x] 마우스 드래그로 360도 3D 회전 및 줌(OrbitControls) 확인

### 완료 조건 (Definition of Done)
* 브라우저에서 축구장 한가운데 각진 마네킹이 서 있고, 포즈 버튼 클릭 시 관절이 해당 포즈로 즉시 꺾이며, 머리 크기 슬라이더와 3D 카메라 회전이 정상 작동함.
...
- [ ] 1:1, 4:5, 9:16 세 화면비에서 화면 캔버스와 다운로드 PNG의 위치/줄바꿈 100% 일치 대조 검증 (T03-C11~C13)
